...
class FusionBrainAPI:
    ...
    def generate(self, prompt, pipeline_id, images=1, width=512, height=512):
        ...
        data = response.json()
        logger.info(
            f"Задача создана: UUID {data['uuid']}, статус: {data.get('status', 'UNKNOWN')}"
        )
        return data["uuid"]
        ...
        while attempts > 0:
            try:
                ...
                if response.status_code == 404:
                    logger.error(
                        "404: Задача не найдена (возможно, истекла или удалена)"
                    )
                    return None
                response.raise_for_status()
                data = response.json()
                ...
                if data["status"] == "DONE":
                    ...
                    logger.error(
                        f"Генерация провалилась: {data.get('errorDescription', 'Unknown error')}"
                    )
                    return None
