...
import time
import requests
from config import FUSIONBRAIN_URL, FUSIONBRAIN_API_KEY, FUSIONBRAIN_SECRET_KEY  # Импорт из твоего config
...
class FusionBrainAPI:
    ...
    def check_availability(self):
        response = requests.get(self.URL + 'key/api/v1/pipeline/availability', headers=self.AUTH_HEADERS)
    ...
    def check_generation(self, uuid, attempts=10, delay=10):
        while attempts > 0:
            response = requests.get(f"{self.URL}key/api/v1/pipeline/status/{uuid}", headers=self.AUTH_HEADERS)
            data = response.json()
